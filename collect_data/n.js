...
async function doGetRequest() {
    try {
        arr_web_time = []
        var response = await axios.get(`https://airlabs.co/api/v9/flights?_fields=
                                    airline_iata,
                                    airline_icao,
                                    flight_number,
                                    flight_icao,
        ...
        console.log("r1:", response.data.response[0].flight_iata);

        response.data.response.forEach(async res => {
            try {
                var time_info = await axios.get(`https://airlabs.co/api/v9/flight?_fields=dep_time, dep_time_utc, dep_estimated_utc, dep_actual_utc,arr_time_utc,arr_estimated_utc,dep_delayed,delayed,arr_delayed,arr_actual_utc,&flight_iata=${response.data.response[0].flight_iata}&api_key=${air_labs_API_KEY}`);
                arr_web_time.push({ "webcite": `https://airlabs.co/api/v9/flight?_fields=dep_time, dep_time_utc, dep_estimated_utc, dep_actual_utc,arr_time_utc,arr_estimated_utc,dep_delayed,arr_delayed,arr_actual_utc,&flight_iata=${response.data.response[0].flight_iata}&api_key=${air_labs_API_KEY}`, "time": lib_complete_info.getNow() });
                ...
                var is_holyday = lib_complete_info.isJewishIsraelyHolyday(time_info.dep_time_utc);
                ...
                const info = {
                    ...
                    "AIRLINE_IATA_CODE": company_code,
                    "FLIGHT_NUMBER": flight_number,
                    "FLIGHT_IATA_CODE": flight_iata,
                    "ORIGIN_AIRPORT_IATA": res.dep_iata,
                    "DESTINATION_AIRPORT_IATA": res.arr_iata,
                    "SCHEDULED_DEPARTURE": scheduled_dep_time,
                    "DEPARTURE_TIME": actual_dep_time,
                    "UPDATED_DEPARTURE_TIME": updated_dep_time,
                    "SCHEDULED_ARRIVAL": scheduled_arr_time,
                    "ARRIVAL_TIME": actual_arr_time,
                    "UPDATED_ARRIVAL_TIME": updated_arr_time,
                    "DEPARTURE_DELAY": time_info.dep_delayed,
                    "ARRIVAL_DELAY": time_info.arr_delayed,
                    "FLIGHT_STATUS": f_status,
                    "DELAY_TYPE": punctuality,
                    "DISTANCE_TYPE": flight_type,
                    "IS_HOLYDAY": is_holyday,
                    "IS_VICATION": is_summer_vacation,
                    "WEATHER_DEP_DESC": weather_dep_desc_desc_code_deg[0],
                    "WEATHER_DEP_DESC_CODE": weather_dep_desc_desc_code_deg[1],
                    "WEATHER_DEP_DEG": weather_dep_desc_desc_code_deg[2],
                    "WEATHER_ARR_DESC": weather_arr_desc_desc_code_deg[0],
                    "WEATHER_ARR_DESC_CODE": weather_arr_desc_desc_code_deg[1],
                    "WEATHER_ARR_DEG": weather_arr_desc_desc_code_deg[2],
                    "DEPARTURE_COUNTRY": depurture_country_name,
